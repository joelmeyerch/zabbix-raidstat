...
build: 
	...
	go build -ldflags=$(LD_FLAGS) -buildmode=plugin -o $(BUILD_DIR)/storcli.so plugins/storcli/main.go
	go build -ldflags=$(LD_FLAGS) -buildmode=plugin -o $(BUILD_DIR)/storcli2.so plugins/storcli2/main.go
...
install: build
	...
	install -m 644 $(BUILD_DIR)/storcli.so /opt/raidstat/storcli.so
	install -m 644 $(BUILD_DIR)/storcli2.so /opt/raidstat/storcli2.so
